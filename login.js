...
const uri = url + path;
    ...
    try{
        ...
        if(responseData.jwttoken != null){
            setCookie(JSON.stringify(responseData), 1);
            window.location.href = "classScheduling_admin.html"
        }
...
function setCookie(token, duration){
    const now = new Date();
    const expiration_Date = new Date(now.getTime() + duration * 60 *10000);
    document.cookie = `Authentication=${token}; expires=${expiration_Date.toUTCString()};`;
...
btn2.addEventListener("click", event => {
    window.location.href = "classScheduling.html";
})